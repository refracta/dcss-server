...
sub main() {
  ...
  if (not $ver) {
    do_prompt 'trunk', 'dcssca', 'hellcrawl', 'gnollcrawl', 'bcrawl', 'bloatcrawl2', 'gooncrawl', 'xcrawl', 'stoatsoup', 'bcadrencrawl', 'kimchicrawl', 'addedcrawl', 'dcst', 'nostalgia', 'yiufcrawl', 'oofcrawl', 'boggartcrawl', 'alphabetsoup', map { "0.$_" } (11 .. 34);
  } elsif ($ver eq 'trunk') {
    do_update 'trunk';
  } elsif ($ver =~ /^0\.\d+$/) {
  ...
  } elsif ($ver eq 'alphabetsoup') {
    do_update 'gcc', 'alphabetsoup alphabetsoup/alphabet_soup-0.27';
  } else {
